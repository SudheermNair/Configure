...
const HotelConfig = () => {
        ...
        if (mod.name === (module ? module.name : "")) {
          const updatedSubmodules = mod.submodules.map((sub) => {
            if (submodules.some((newSub) => newSub.name === sub.name)) {
              ...
              // Include options if enabled
              const updatedOptions = hasOptions ? options : [];
              return {
                ...sub,
  ...
  // Handle changes to option fields
        ...
        {selectedHotel && (
          <>
            ...
            {selectedModule && (
              <div className="dropdown-container submodules">
                <label>Submodules:</label>
                <div className="submodule-selection">
                  <select
                    className="submodule-dropdown"
                    onChange={handleSubmoduleSelect}
                  >
                ...
                <label>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!e.target.checked) {
                        setKeyValuePairs({});
                        setHasOptions(false); // Reset hasOptions when details are disabled
                        setOptions([{ key: "", value: "" }]); // Reset options
                      }
                    }}
                  />
                  Has Details
