...
const FieldSelected = ({ data = [], setData }) => {
  ...
  const handleSubmit = () => {
    ...
    const textData = JSON.stringify(data, (key, value) => {
      if (key === 'keys') return undefined; // Exclude 'keys' property
      return value;
    }, 2);
      ...
      <div className="selected-json">
        <pre className="selected-json-container">
          {JSON.stringify(data, (key, value) => {
            if (key === 'keys') return undefined; // Exclude 'keys' from display
            return value;
          }, 2)}
        </pre>
      </div>
            ...
            {hotel.modules &&
              ...
              hotel.modules.map((module, moduleIndex) => (
                <div key={moduleIndex} className="hotel-sub-info">
                  ...
                  {Object.keys(module)
                    .filter((key) => key !== "name" && key !== "submodules") // Exclude 'name' and 'submodules'
                    .map((key) => (
                      <div key={key}>
                        {`${key}: ${module[key]}`}
                        <button
                  ...
                  {module.submodules.map((submodule, submoduleIndex) => (
                    <div key={submoduleIndex} className="submodule-info">
                      ...
                      {Object.keys(submodule)
                        .filter((key) => key !== "name") // Filter out 'name'
                        .map((key) => (
                          <div key={key}>
                            {`${key}: ${submodule[key]}`}
                            <button
